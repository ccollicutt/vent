...
class PacketInventory(object):
    ...
    def add_device(self, device, project):
        ...
        for ip_address in device.ip_addresses:
            if ip_address['public'] is True and ip_address['address_family'] == 4:
                ansible_host = ip_address['address']
    ...
    def get_host_info_dict_from_device(self, device):
        ...
        # NOTE(curtis): I have moved this here b/c I want the inventory_hostname
        # to be the hostname, not the IP address, and instead set the
        # ansible_host to be the public IP address
        ansible_host = None
        for ip_address in device.ip_addresses:
            if ip_address['public'] is True and ip_address['address_family'] == 4:
                ansible_host = ip_address['address']

        device_vars['ansible_host'] = ansible_host
